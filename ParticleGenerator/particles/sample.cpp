...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Corey McFarland" };
...
Display( )
{
	...
	glLoadIdentity( );
	gluLookAt( 1000., -1000., 1000.,     0., -100., 0.,     0., -1., 0. );
...
InitGlui( )
{
	...
	Glui->add_checkbox( "Show Wireframe",   &WiresOn );
...
InitLists( )
{
	...
	// ****************************************
	// Ensure sphere parameters are reflected in particles.cl
	// ****************************************

	// Sphere 1

		glColor3f( 0.9f, 0.f, 0.f );	// 0. <= r,g,b <= 1.
		glPushMatrix( );
			glTranslatef( -850.f, 0.f, 0.f );
			glutWireSphere( 600.f, 20, 20 );
		glPopMatrix( );
	...
	// Sphere 3
		...
		glColor3f( 0.f, 0.9f, 0.f );	// 0. <= r,g,b <= 1.
		glPushMatrix();
			glTranslatef(0.f, 850.f, 0.f);
			glutWireSphere(600.f, 20, 20);
		glPopMatrix();

	// Sphere 4

		glColor3f(0.f, 0.f, 0.9f);	// 0. <= r,g,b <= 1.
		glPushMatrix();
			glTranslatef(0.f, 0.f, -825.f);
			glutWireSphere(600.f, 20, 20);
		glPopMatrix();
...
ResetParticles( )
{
	...
	for( int i = 0; i < NUM_PARTICLES; i++ )
	{
		...
		while (!circle) {
			...
			pythag = (points[i].x * points[i].x) + (points[i].y * points[i].y) + (points[i].z * points[i].z);

			if (pythag <= r * r && pythag >= 1) {
				circle = true;
			}
		}
	}
	...
	{
		m = Ranf(MMIN, MMAX);
		hVel[i].x = points[i].x / RMAX * m;
		hVel[i].y = points[i].y / RMAX * m;
		hVel[i].z = points[i].z / RMAX * m;
	}
	...
	for( int i = 0; i < NUM_PARTICLES; i++ )
	{
		colors[i].r = abs(hVel[i].x) / MMAX;
		colors[i].g = abs(hVel[i].y) / MMAX;
		colors[i].b = abs(hVel[i].z) / MMAX;
		colors[i].a = 1.;
	}
	glUnmapBuffer( GL_ARRAY_BUFFER );
